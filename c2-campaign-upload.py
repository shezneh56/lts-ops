#!/usr/bin/env python3
"""Create C2 Experience campaigns in EmailBison."""
import json
import time
import httpx
...
CENTRAL_BANK = {
    ...
    "step1_variations": [
        # Variation 1 - Account Opening Journey (main)
        {
        ...
        # Variation 2 - Product Navigation
        {
...
CCC_NEBRASKA = {
    ...
    "step1_variations": [
        # Variation 1 - Enrollment Journey (main)
        {
...
CERTINIA = {
    ...
    "step1_variations": [
        # Variation 1 - Product Discovery (main)
        {
        ...
        # Variation 3 - Self-Service Portal
        {
...
def create_campaign(campaign_data: dict) -> dict:
    ...
    with httpx.Client(timeout=30.0, headers=headers) as client:
        ...
        # Step 2: Build sequence steps
        sequence_steps = []
        step1_main_id = None
        
        # Add Step 1 variations
        for i, var in enumerate(campaign_data["step1_variations"]):
            step = {
        ...
        sequence_steps.append({
            ...
            "order": len(campaign_data["step1_variations"]) + 1,
            "wait_in_days": step2["wait_in_days"],
            "variant": False,
            "thread_reply": step2["thread_reply"]
        })
        ...
        resp.raise_for_status()
        sequence_data = resp.json()["data"]
        sequence_id = sequence_data["id"]
        
        print(f"  ✓ Created sequence ID: {sequence_id}")
        print(f"  ✓ Created {len(sequence_data['sequence_steps'])} steps:")
        
        # Get the first step ID for setting up variants
        ...
        for step in steps:
            ...
            print(f"    - Step {step['order']}: ID {step['id']}{variant_marker}{thread_marker}")
        
        # Step 4: Update variants to link to first step
        # Need to PUT to set variant_from_step_id properly
        print("  → Linking A/B variants to main step...")
        ...
        for i, step in enumerate(steps):
            ...
            # Link variants to first step (skip first step and follow-up)
            if step.get("variant") and i > 0 and i < len(steps) - 1:
        ...
        resp.raise_for_status()
        print("  ✓ Variants linked successfully")
...
def main():
    ...
    print("\n" + "=" * 60)
    print("C2 EXPERIENCE - EMAILBISON CAMPAIGN CREATION")
    print("=" * 60)
    
    results = []
    
    # Create campaigns with small delay between
